...
const PromptModelPanelInner: React.FC<PromptModelPanelProps> = ({
  ...
  const referenceLabel = isHousingFlow ? "空间参考" : "产品";
  const workflowMode = isHousingFlow
    ? "房屋设计"
    : selectedModel
      ? "锁定模特"
      : settings.autoUseLastImage
        ? "连续编辑"
        : "全新生成";
  const workflowHint = isHousingFlow
    ? "以建筑/室内为目标创作。建议上传空间参考图，描述结构、材质、光照和风格。"
    : selectedModel
      ...
      <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
        <div className="flex flex-wrap items-center gap-3 min-w-0">
          <div className="flex items-center gap-1.5 rounded-lg border border-dark-700 bg-dark-800/50 px-2.5 py-1.5">
            <span className="text-[11px] text-gray-400 shrink-0">创作方向</span>
            <button
          ...
          <div className="flex flex-col gap-1.5 rounded-lg border border-dark-700 bg-dark-800/50 px-2.5 py-1.5">
            <div className="flex items-center gap-2">
              <span className="text-[11px] text-gray-400 shrink-0">{referenceLabel}</span>
              <div
                ref={productSlotRef}
                role="button"
                aria-label={isHousingFlow ? "空间参考图输入位" : "产品图输入位"}
                tabIndex={0}
                ...
                title={settings.productImage ? `${referenceLabel}已设置` : `点击上传或粘贴${referenceLabel}`}
              >
                {settings.productImage ? (
                  <>
                    <img
              ...
              <button
                ...
                title={settings.productImage ? `更换${referenceLabel}` : `上传${referenceLabel}`}
              >
            ...
            <p className={`text-[10px] leading-relaxed ${
              ...
              {settings.productImage
                ? `${referenceLabel}已就绪，可继续更换；也可先点“粘贴${referenceLabel}”后按 Cmd/Ctrl + V 覆盖。`
                : awaitingProductPaste
                  ? `现在直接按 Cmd/Ctrl + V 即可把剪贴板图片放进${referenceLabel}位。`
                  : `先点“上传”选择文件，或点“粘贴${referenceLabel}”后按 Cmd/Ctrl + V。`}
            </p>
          </div>
